...
import { URL_TYPE_API, URL_TYPE_MAIN } from "@/constants";
...
|--------|-------|----------|-------|
| `Authorization` | `Bearer YOUR_JWT_TOKEN` | **Yes** | JWT token from the [Issue JWT token](#issue-jwt-token) response field `data.token`. |
...
These endpoints are available only for B2B partner services, you need to fill the form <RegionalUrl type={URL_TYPE_MAIN} path="/institutional-services/b2b">institutional services form</RegionalUrl> in order to get permissions to use these endpoints.
