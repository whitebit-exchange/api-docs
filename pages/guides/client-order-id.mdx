...
<Card className="mt-6">
  <CardContent className="pt-6 text-foreground/90">
    ...
    **Important:** While you can choose your own `clientOrderId`, it must be unique for any orders placed by your account. This means you cannot reuse the same `clientOrderId` for a new order if that ID was already used for another order.
  </CardContent>
</Card>
