...
<Card className="mt-6">
  <CardContent className="pt-6 text-foreground/90">
    The `clientOrderId` is your own optional ID to help you keep track of your orders. Think of it as a custom label you can create. This ID works alongside the standard `orderId` provided by the exchange, giving you another way to manage and follow your orders.

    **Important:** While you can choose your own `clientOrderId`, it must be unique for any orders placed within a 24-hour period. This means you cannot reuse the same `clientOrderId` for a new order if that ID was already used for another order in the preceding 24 hours.
  </CardContent>
</Card>
