...
function applyUpdates(orderBookSide: IDepth[], updates: IDepth[] | undefined) {
...
def handle_depth_update(params):
    ...
    if full_reload:
        ...
        apply_updates(order_book.asks, update_data.get('asks', []))
        apply_updates(order_book.bids, update_data.get('bids', []))
...
public class OrderBookClient {
    ...
    private static void applyUpdates(List<IDepth> orderBookSide, JsonArray updates) {
        for (JsonValue value : updates) {
            ...
            boolean found = false;
            for (IDepth entry : orderBookSide) {
                if (entry.getPrice().equals(price)) {
                    if ("0".equals(amount)) {
                        orderBookSide.remove(entry);
                    } else {
