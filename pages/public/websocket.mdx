...
function applyUpdates(orderBookSide: Hash<IDepth[]>, updates: Hash<IDepth[]>) {
  updates.forEach(([price, amount]) => {
    ...
    if (amount === '0') {
      ...
      if (priceIndex !== -1) {
        ...
        // Add a new price level to the order book.
...
function truncateOrderBook(orderBookSide: Hash<IDepth[]>) {
...
def handle_depth_update(params):
    ...
    if full_reload:
        order_book.asks = {price: amount for price, amount in update_data['asks']}
        order_book.bids = {price: amount for price, amount in update_data['bids']}
    else:
        apply_updates(order_book.asks, update_data['asks'])
        apply_updates(order_book.bids, update_data['bids'])
        truncate_order_book(order_book.asks)
        truncate_order_book(order_book.bids)

def apply_updates(order_book_side, updates):
    for price, amount in updates:
        if amount == '0':
            order_book_side.pop(price, None)
        else:
            order_book_side[price] = amount

def truncate_order_book(order_book_side):
    if len(order_book_side) > LIMIT:
        keys_to_remove = list(order_book_side.keys())[LIMIT:]
        for key in keys_to_remove:
            order_book_side.pop(key)
...
public class OrderBookClient {
    ...
    private static Map<String, IDepth> asks = new HashMap<>();
    private static Map<String, IDepth> bids = new HashMap<>();
    ...
    public void onOpen(Session session) throws Exception {
        JsonObject message = Json.createObjectBuilder()
                ...
                .add("params", Json.createArrayBuilder()
                        ...
                        .add(true))
                .build();
        try (JsonWriter writer = Json.createWriter(session.getBasicRemote().getSendStream())) {
    ...
    public void onMessage(String message, Session session) {
        try (JsonReader reader = Json.createReader(() -> new ByteArrayInputStream(message.getBytes(StandardCharsets.UTF_8)))) {
            ...
            if ("depth_update".equals(jsonMessage.getString("method"))) {
                ...
                if (fullReload) {
                    ...
                    applyUpdates(asks, params.getJsonObject(1).getJsonArray("asks"));
                    applyUpdates(bids, params.getJsonObject(1).getJsonArray("bids"));
    ...
    private static Map<String, IDepth> parseOrderBookSide(JsonArray jsonArray) {
        Map<String, IDepth> orderBookSide = new HashMap<>();
        for (JsonValue value : jsonArray) {
            JsonArray level = value.asJsonArray();
            String price = level.getString(0);
            String amount = level.getString(1);
            orderBookSide.put(price, new IDepth(price, amount));
        }
        return orderBookSide;
    }

    private static void applyUpdates(Map<String, IDepth> orderBookSide, JsonArray updates) {
    ...
    private static void truncateOrderBook(Map<String, IDepth> orderBookSide) {
        if (orderBookSide.size() > LIMIT) {
            // Only keep the most recent LIMIT entries
            while (orderBookSide.size() > LIMIT) {
                String firstKey = orderBookSide.keySet().iterator().next();
                orderBookSide.remove(firstKey);
            }
        }
    }
